...
__export(src_exports, {
  ...
  ExperimentalHNSWIndex: () => ExperimentalHNSWIndex,
  getEmbedding: () => getEmbedding,
  initializeModel: () => initializeModel
});
module.exports = __toCommonJS(src_exports);
var import_fastpriorityqueue = __toESM(require("fastpriorityqueue"));
...
var _Cache = class {
  ...
  static getInstance(max = 1e4, maxAge = 1e3 * 60 * 10) {
    if (!_Cache.instance) {
      ...
      _Cache.instance = new import_lru_cache.LRUCache(options);
    }
...
var ExperimentalHNSWIndex = class {
  ...
  toBinary() {
    return (0, import_msgpack.encode)({
      L: this.L,
      mL: this.mL,
      efc: this.efc,
      index: this.index
  ...
  static fromBinary(binary) {
    const data = (0, import_msgpack.decode)(binary);
...
var EmbeddingIndex = class {
 ...
 Supported storage types: "indexedDB"`
  ...
  async performCompareSearch(dedupeEntries, filter, objectsToSearch, queryEmbedding, topK) {
    const queue = new import_fastpriorityqueue.default(
